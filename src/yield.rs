...
impl YieldScanner {
    ...
    async fn fetch_lido(&self, chain: &Chain) -> Result<Vec<YieldOpportunity>> {
        ...
        let data: LidoAprResponse = resp.json().await.map_err(|e| {
            RouterError::ProtocolFetchFailed {
                protocol: "Lido".into(),
                reason: e.to_string(),
            }
        ...
        let apy = Decimal::from_f64(data.data.smaApr / 100.0).unwrap_or_default();
        ...
        Ok(vec![YieldOpportunity {
            ...
            apy,
            tvl_usd: Decimal::new(30_000_000_000, 0),
            available_liquidity_usd: Decimal::new(1_000_000_000, 0),
            risk_tier: RiskTier::Low,
            has_keyring_gate: false,
            required_policy_id: None,
            fetched_at: Utc::now(),
